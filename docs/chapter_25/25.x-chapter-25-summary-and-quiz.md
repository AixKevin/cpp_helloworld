...
Alex November 23, 2016, 2:59 pm PST February 19, 2025
...
#### 1a)
**Problem:** Base::getName() wasn't made virtual, so b.getName() doesn't resolve to Derived::getName().

#### 1b)
**Problem:** Base::getName() is non-const and Derived::getName() is const, so Derived::getName() is not considered an override.

#### 1c)
**Problem:** d was assigned to b by value, causing d to get sliced.

#### 1d)
**Problem:** Base was declared as final, so Derived can't be derived from it. This will cause a compile error.

#### 1e)
**Problem:** Derived::getName() is a pure virtual function (with a body), and therefore Derived is an abstract class that can't be instantiated.

#### 1f)
**Problem:** This program actually produces the right output, but has a different issue. We're deleting b, which is a Base pointer, but we never added a virtual destructor to the Base class. Consequently, the program only deletes the Base portion of the Derived object, and the Derived portion is left as leaked memory.
...
Derive two classes from Shape: a Triangle, and a Circle. The Triangle should have 3 Points as members. The Circle should have one center Point, and an integer radius.
...
Given the above classes (Point, Shape, Circle, and Triangle), finish the following program to print each shape and find the largest radius.
...
### Question 2d (Extra credit)

Update the prior solution to use a `std::vector<std::unique_ptr<Shape>>`.
...
// ... Point, Shape, Triangle, Circle classes same as above ...
