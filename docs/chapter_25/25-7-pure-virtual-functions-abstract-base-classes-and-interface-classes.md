...
Alex February 13, 2008, 4:09 pm PST October 1, 2024
...
In this case, speak() is still considered a pure virtual function because of the "= 0" (even though it has a definition) and Animal is still considered an abstract base class (and thus can't be instantiated). Any class that inherits from Animal needs to provide its own definition for speak() or it will also be considered an abstract base class.
